...
func _physics_process(_delta: float) -> void:
	if Input.is_action_pressed("move_forward"):
		ForceUtils.apply_and_display_force(
			self, Vector3.UP * 40, Vector3(2, 0, 1), Color(1, 1, 0), "push", 0.005
		)
	#prints(position.y)
	#var f = Vector3(0., -position.y * 200., 0.)
	#ForceUtils.apply_and_display_force(self, f)
	var i = 0
	for p in probes:
		i += 1
		var depth = -p.global_position.y + 0.5
		if depth > 0:
			ForceUtils.display_vector(self, p.position, Vector3.ZERO, Color(1, 0, 0), "pos %d" % i)
			ForceUtils.apply_and_display_force(
				self,
	...
	#ForceUtils.apply_and_display_force(boom, Vector3.FORWARD*0.1, Vector3.LEFT, Color(0,1,0), "wind")
	boom.angular_velocity = boom.angular_velocity * 0.1
	#ForceUtils.display_vector(boom, Vector3.FORWARD, Vector3.RIGHT*0.4 , Color(0,0,1), "wind2")
	#ForceUtils.apply_and_display_force(boom, Vector3.FORWARD*0.1,
#	Vector3.RIGHT*2.9 , Color(0,0,1), "wind2", 10)

	var offset: float = Input.get_axis("turn_left", "turn_right")
	ForceUtils.apply_and_display_force(
		boom, boom.basis.z * 0.1 * offset, boom.basis.y, Color(1, 1, 0), "wind2", 10
	)
	#ForceUtils.
